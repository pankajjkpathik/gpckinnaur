    ...
    <section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {tiles.map((t, i) => (
          ...
          >
            <div className="absolute inset-0 flex items-center justify-center text-white/40 text-5xl">{i + 1}</div>
            <div className="absolute inset-0 bg-[color:var(--navy)]/0 group-hover:bg-[color:var(--navy)]/80 transition-all flex items-center justify-center">
              <span className="text-white opacity-0 group-hover:opacity-100 font-semibold">{t}</span>
            </div>
          </div>
        ))}
      </div>
