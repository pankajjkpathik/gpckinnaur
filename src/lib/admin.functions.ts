...
export const listAssignments = createServerFn({ method: "GET" })
  .inputValidator((d) =>
    z.object({ staff_id: z.number().int().optional(), academic_year: z.string().optional() }).parse(d ?? {}),
  )
  .handler(async ({ data }) => {
    await requireStaff();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    let q = supabaseAdmin
      ...
      .select(
        "id, staff_id, subject_id, branch, semester, academic_year, guest_faculty, guest_institute, staff_users(username,department), subjects(code,name)",
      )
      .order("academic_year", { ascending: false });
...
export const publishTimetable = createServerFn({ method: "POST" })
  .inputValidator((d) =>
    z
      .object({ branch: z.string(), semester: z.number().int(), academic_year: z.string(), published: z.boolean() })
      .parse(d),
  )
  .handler(async ({ data }) => {
    await requireRole(adminRoles);
    ...
    data.rows.forEach((raw, i) => {
      try {
        ...
        const norm = {
          ...
          branch: String(pick(raw, "branch", "Branch") ?? "")
            .trim()
            .toLowerCase(),
          semester: numOr0(pick(raw, "semester", "Semester", "sem")),
          kind: String(pick(raw, "kind", "Kind", "theory_practical") ?? "theory")
            .trim()
            .toLowerCase(),
      ...
      try {
        ...
        parsed.push(
          syllabusRowSchema.parse({
            ...
            branch: String(raw.branch ?? raw.Branch ?? "")
              .trim()
              .toLowerCase(),
...
export const bulkImportPeriods = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    data.rows.forEach((raw, i) => {
      try {
        const norm = periodRowSchema.parse({
          ...
          start_time: String(raw.start_time ?? raw.Start ?? "")
            .trim()
            .slice(0, 8),
          end_time: String(raw.end_time ?? raw.End ?? "")
            .trim()
            .slice(0, 8),
    ...
    if (valid.length) {
      const { error, count } = await supabaseAdmin
        .from("periods_master")
        .upsert(valid, { onConflict: "period_no", count: "exact" });
      if (error) throw new Error(error.message);
      inserted = count ?? valid.length;
    }
    return { inserted, errors };
...
export const bulkImportAssignments = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    parsed.forEach((r, i) => {
      ...
      if (!staff_id) {
        errors.push({ row: i + 2, error: `Faculty not found: ${r.username}` });
        return;
      }
      if (!subject_id) {
        errors.push({ row: i + 2, error: `Subject not found: ${r.subject_code} (${r.branch} sem ${r.semester})` });
  ...
  .handler(async ({ data }) => {
    ...
    parsed.forEach((r, i) => {
      ...
      if (r.subject_code) {
        ...
        if (!subject_id) {
          errors.push({ row: i + 2, error: `Subject not found: ${r.subject_code}` });
          return;
        }
      }
      if (r.username) {
