...
// ============ DASHBOARD ============
export const studentDashboard = createServerFn({ method: "GET" })
  .handler(async () => {
    ...
    const [att, todayPeriods, pendingLeaves] = await Promise.all([
      ...
      supabaseAdmin.from("timetable")
        ...
        .eq("branch", me.branch).eq("semester", me.semester)
        .eq("day_of_week", dow).eq("published", true).order("period_no"),
      supabaseAdmin.from("leave_applications").select("id", { count: "exact", head: true })
        .eq("applicant_id", me.id).eq("applicant_type", "student").eq("status", "pending"),
    ]);
    const all = att.data ?? [];
...
// ============ ATTENDANCE ============
export const studentAttendance = createServerFn({ method: "GET" })
  .handler(async () => {
    const me = await requireStudent();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data, error } = await supabaseAdmin
      ...
      .eq("student_id", me.id).order("date", { ascending: false }).limit(500);
    ...
    return {
      ...
      by_subject: Array.from(subj.values()).map((s) => ({
        ...s, pct: s.total ? Math.round((s.present / s.total) * 1000) / 10 : 0,
      })),
    };
  });
...
export const studentMarks = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const me = await requireStudent();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    let q = supabaseAdmin.from("marks")
      .select("exam_type, obtained, max_marks, academic_year, approved_by_hod, subjects(code,name)")
      .eq("student_id", me.id).eq("approved_by_hod", true);
...
// ============ TIMETABLE ============
export const studentTimetable = createServerFn({ method: "GET" })
  .handler(async () => {
    const me = await requireStudent();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const [tt, periods] = await Promise.all([
      supabaseAdmin.from("timetable")
        ...
        .eq("branch", me.branch).eq("semester", me.semester).eq("published", true)
...
// ============ SYLLABUS ============
export const studentSyllabus = createServerFn({ method: "GET" })
  .handler(async () => {
    const me = await requireStudent();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: subjects } = await supabaseAdmin.from("subjects")
      .select("id, code, name")
      .eq("branch", me.branch).eq("semester", me.semester).order("code");
    const ids = (subjects ?? []).map((s: any) => s.id);
    if (ids.length === 0) return [];
    const { data: units } = await supabaseAdmin.from("syllabus_units")
      .select("subject_id, unit_no, title, topics").in("subject_id", ids).order("unit_no");
    return (subjects ?? []).map((s: any) => ({
      ...s, units: (units ?? []).filter((u: any) => u.subject_id === s.id),
    }));
  });

// ============ LEAVE ============
export const studentMyLeaves = createServerFn({ method: "GET" })
  .handler(async () => {
    const me = await requireStudent();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data, error } = await supabaseAdmin
      .from("leave_applications").select("*")
      .eq("applicant_id", me.id).eq("applicant_type", "student")
...
export const studentApplyLeave = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error } = await supabaseAdmin.from("leave_applications").insert({
      applicant_id: me.id, applicant_type: "student",
      leave_type: data.leave_type, from_date: data.from_date, to_date: data.to_date,
...
export const studentCancelLeave = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const me = await requireStudent();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await supabaseAdmin.from("leave_applications").delete()
      .eq("id", data.id).eq("applicant_id", me.id).eq("applicant_type", "student").eq("status", "pending");
    if (error) throw new Error(error.message);
    return { ok: true };
  });

// ============ ACADEMIC CALENDAR ============
export const studentCalendar = createServerFn({ method: "GET" })
  .handler(async () => {
    await requireStudent();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const today = new Date().toISOString().slice(0, 10);
    const { data, error } = await supabaseAdmin.from("academic_calendar")
      .select("*").gte("end_date", today).order("start_date").limit(50);
    if (error) throw new Error(error.message);
    return data ?? [];
  });

// ============ CIRCULARS ============
export const studentCirculars = createServerFn({ method: "GET" })
  .handler(async () => {
    await requireStudent();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data, error } = await supabaseAdmin.from("circulars")
      .select("*").in("audience", ["all", "students"])
