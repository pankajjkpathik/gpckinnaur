...
// ============ CLASS SESSIONAL REPORT (read-only) ============
// Returns the mid-sessional or final-sessional report for the ENTIRE class
// (student's own branch + semester). Each row = one student, with per-subject
// obtained marks. Only approved-by-HOD marks are included.
export const studentSessionalReport = createServerFn({ method: "GET" })
  .inputValidator((d) =>
    z
      .object({
        exam_type: z.enum(["mid_sessional", "final_sessional"]),
  ...
  .handler(async ({ data }) => {
    ...
    // 3. marks (approved) for this exam type
    let marksQ = supabaseAdmin
      .from("marks")
      .select("student_id, subject_id, obtained, max_marks, academic_year")
      .eq("exam_type", data.exam_type)
      .eq("approved_by_hod", true);
    if (subjectIds.length > 0) marksQ = marksQ.in("subject_id", subjectIds);
    if (data.academic_year) marksQ = marksQ.eq("academic_year", data.academic_year);
    const { data: marks } = await marksQ;

    // Index: student_id -> subject_id -> {obtained, max}
    const byStudent = new Map<number, Map<number, { obtained: number; max: number }>>();
    (marks ?? []).forEach((m: any) => {
      if (!byStudent.has(m.student_id)) byStudent.set(m.student_id, new Map());
      byStudent.get(m.student_id)!.set(m.subject_id, {
    ...
    return {
      ...
      students: (students ?? []).map((s: any) => {
        const row = byStudent.get(s.id) ?? new Map();
        const per_subject = (subjects ?? []).map((subj: any) => {
          const cell = row.get(subj.id);
          return {
            subject_id: subj.id,
        ...
        const total = per_subject.reduce((a, x) => a + (x.obtained ?? 0), 0);
        const total_max = per_subject.reduce((a, x) => a + (x.max ?? 0), 0);
        return {
          id: s.id,
