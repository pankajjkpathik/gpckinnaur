    ...
    <PortalShell title="Clerk Portal" subtitle="Master Records" me={me as any} accent="amber">
      <div className="container mx-auto px-4 py-6 space-y-4">
        <div className="flex gap-1 border-b flex-wrap">
          {([["students","Students"],["import","Bulk Import"],["promote","Promote"],["salary","Salary"]] as [Tab,string][]).map(([k,l]) => (
  ...
  return (
    <div className="space-y-3 max-w-3xl">
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={downloadSampleXlsx} className="border px-3 py-2 rounded text-sm inline-flex items-center gap-1">
          <FileSpreadsheet className="w-4 h-4" /> Download Sample.xlsx
        </button>
        <span className="text-xs text-muted-foreground">Required: <code>enrollment_no, name, branch, semester, batch_year</code>. Optional: <code>father_name, email, phone</code>.</span>
      </div>
      ...
      <div className="bg-white border rounded p-3 space-y-2">
        ...
        {fileName && <p className="text-xs text-muted-foreground">Loaded <b>{fileName}</b> — {rows.length} rows parsed.</p>}
      </div>
      ...
      <div className="flex gap-2 items-center">
        ...
        <button disabled={!rows.length || !pw || m.isPending} onClick={() => m.mutate({ defaultPassword: pw, rows })} className="bg-amber-600 text-white px-4 py-2 rounded text-sm font-semibold inline-flex items-center gap-1 disabled:opacity-50">
...
function SalaryTab() {
  ...
  const listQ = useQuery({ queryKey: ["salary-list", month, year], queryFn: () => salaryList({ data: { month, year } }) });
  const upsert = useMutation({
    mutationFn: (d: any) => salaryUpsert({ data: d }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["salary-list"] }); setAdding(false); },
  });
  ...
  const total = useMemo(() => (listQ.data ?? []).reduce((s: number, r: any) => s + Number(r.net_pay || 0), 0), [listQ.data]);

  const exportXlsx = () => {
    const rows = (listQ.data ?? []).map((r: any) => ({
      Staff: r.staff_users?.username, Role: r.staff_users?.role, Dept: r.staff_users?.department,
      Month: r.month, Year: r.year, Basic: r.basic, DA: r.da, HRA: r.hra,
      "Other Allow": r.other_allow, Deductions: r.deductions, "Net Pay": r.net_pay,
      "Paid On": r.paid_on, Remarks: r.remarks,
    }));
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        <select value={month} onChange={(e) => setMonth(Number(e.target.value))} className="border rounded px-2 py-2 text-sm bg-white">
          {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (
            <option key={m} value={m}>{new Date(2000, m - 1, 1).toLocaleString("en", { month: "long" })}</option>
          ))}
        </select>
        <input type="number" min={2000} max={2100} value={year} onChange={(e) => setYear(Number(e.target.value))} className="border rounded px-2 py-2 text-sm w-24" />
        <button onClick={() => setAdding(true)} className="bg-amber-600 text-white px-3 py-2 rounded text-sm font-semibold inline-flex items-center gap-1"><Plus className="w-4 h-4" /> Salary Entry</button>
        <button onClick={exportXlsx} disabled={!listQ.data?.length} className="border px-3 py-2 rounded text-sm inline-flex items-center gap-1 disabled:opacity-50"><Download className="w-4 h-4" /> Export</button>
        <span className="ml-auto text-sm">Total Net: <b>₹{total.toLocaleString("en-IN")}</b></span>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {(listQ.data ?? []).map((r: any) => (
              <tr key={r.id} className="border-t">
                <td className="px-3 py-2"><b>{r.staff_users?.username}</b><div className="text-xs text-muted-foreground">{r.staff_users?.role}</div></td>
                ...
                <td className="px-3 py-2 text-right">
                  <button onClick={() => { if (confirm("Delete this salary entry?")) del.mutate({ id: r.id }); }} className="p-1.5 hover:bg-secondary rounded text-rose-700"><Trash2 className="w-4 h-4" /></button>
                </td>
              </tr>
            ))}
            {(listQ.data ?? []).length === 0 && <tr><td colSpan={9} className="text-center py-6 text-muted-foreground">No salary entries for {month}/{year}.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {adding && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setAdding(false)}>
          <div className="bg-white rounded-lg max-w-lg w-full p-5" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-bold text-[color:var(--navy)] mb-3">Add / Update Salary — {month}/{year}</h3>
            <form onSubmit={(e) => {
              e.preventDefault();
              const f = new FormData(e.currentTarget);
              upsert.mutate({
              ...
              <select name="staff_id" required className="col-span-2 border rounded px-3 py-2 bg-white">
                ...
                {(staffQ.data ?? []).map((s: any) => <option key={s.id} value={s.id}>{s.username} ({s.role}{s.department ? `, ${s.department}` : ""})</option>)}
              </select>
              <label className="text-xs">Basic<input name="basic" type="number" step="0.01" min={0} required className="w-full border rounded px-2 py-1.5" /></label>
              <label className="text-xs">DA<input name="da" type="number" step="0.01" min={0} defaultValue={0} className="w-full border rounded px-2 py-1.5" /></label>
              <label className="text-xs">HRA<input name="hra" type="number" step="0.01" min={0} defaultValue={0} className="w-full border rounded px-2 py-1.5" /></label>
              <label className="text-xs">Other Allowances<input name="other_allow" type="number" step="0.01" min={0} defaultValue={0} className="w-full border rounded px-2 py-1.5" /></label>
              <label className="text-xs col-span-2">Deductions<input name="deductions" type="number" step="0.01" min={0} defaultValue={0} className="w-full border rounded px-2 py-1.5" /></label>
              <label className="text-xs">Paid On<input name="paid_on" type="date" className="w-full border rounded px-2 py-1.5" /></label>
              <label className="text-xs col-span-2">Remarks<input name="remarks" className="w-full border rounded px-2 py-1.5" /></label>
              {upsert.error && <p className="col-span-2 text-xs text-destructive">{upsert.error.message}</p>}
              <div className="col-span-2 flex justify-end gap-2 pt-1">
                <button type="button" onClick={() => setAdding(false)} className="px-3 py-1.5 border rounded">Cancel</button>
                <button disabled={upsert.isPending} className="px-4 py-1.5 bg-amber-600 text-white rounded disabled:opacity-50">{upsert.isPending ? "Saving…" : "Save"}</button>
              </div>
            </form>
          </div>
        </div>
