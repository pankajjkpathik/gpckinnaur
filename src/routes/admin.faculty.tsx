...
function FacultyManagement() {
  ...
  const rows = (listQ.data ?? []).filter((f: any) => {
    ...
    return (f.name || "").toLowerCase().includes(s) || (f.username || "").toLowerCase().includes(s) ||
      (f.designation || "").toLowerCase().includes(s) || (f.department || "").toLowerCase().includes(s) ||
      (f.pmis_number || "").toLowerCase().includes(s) || (f.ip_number || "").toLowerCase().includes(s);
  });
    ...
    <PortalShell title="Faculty Management" subtitle="Department teaching staff records" me={me as any} accent="rose">
      <div className="container mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <button onClick={() => setCreating(true)} className="bg-[#7b1f4c] text-white px-4 py-2 rounded text-sm font-semibold flex items-center gap-1.5">
        ...
        <div className="bg-white border rounded-lg p-5">
          <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
            ...
            <div className="relative">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, PMIS, dept…" className="border rounded pl-8 pr-3 py-2 text-sm w-64" />
            </div>
          </div>
                ...
                {rows.map((f: any) => (
                  <tr key={f.id} className="border-t">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-gray-400">@{f.username} · {f.role}{f.is_active ? "" : " · inactive"}</p>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2 items-center">
                        <button onClick={() => setEditing(f)} className="text-gray-500 hover:text-gray-700" title="Edit"><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => { const p = prompt(`New password for ${f.username} (min 8 chars):`); if (p) resetPw.mutate({ id: f.id, newPassword: p }); }} className="text-gray-500 hover:text-gray-700" title="Reset password"><KeyRound className="w-4 h-4" /></button>
                        <button onClick={() => { if (confirm(`Delete ${f.name || f.username}? This removes their login too.`)) del.mutate(f.id); }} className="text-rose-500 hover:text-rose-700" title="Delete"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {rows.length === 0 && (
                  <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">{listQ.isLoading ? "Loading…" : "No faculty found."}</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {(creating || editing) && (
        <FacultyForm
          ...
          onSaved={() => { qc.invalidateQueries({ queryKey: ["faculty-list"] }); setCreating(false); setEditing(null); }}
...
function FacultyForm({ initial, onClose, onSaved, canPrincipal }: { initial: Faculty | null; onClose: () => void; onSaved: () => void; canPrincipal: boolean }) {
      ...
      <div className="bg-white rounded-lg w-full max-w-3xl my-8" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} className="p-5 space-y-4 text-sm">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <Field label="Name"><input name="name" defaultValue={v.name ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Designation"><input name="designation" defaultValue={v.designation ?? ""} placeholder="e.g. Lecturer" className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Department">
              <select name="department" defaultValue={v.department ?? ""} className="border rounded w-full px-3 py-2 bg-white">
                <option value="">—</option>
            ...
            <Field label="IP Number"><input name="ip_number" defaultValue={v.ip_number ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="PMIS Number"><input name="pmis_number" defaultValue={v.pmis_number ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Phone Number"><input name="phone" defaultValue={v.phone ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Email"><input name="email" type="email" defaultValue={v.email ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Last Salary Drawn (₹)"><input name="last_salary_drawn" type="number" step="0.01" defaultValue={v.last_salary_drawn ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Date of Joining"><input name="date_of_joining" type="date" defaultValue={v.date_of_joining ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Date of Retirement"><input name="date_of_retirement" type="date" defaultValue={v.date_of_retirement ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Role">
              <select name="role" defaultValue={v.role ?? "faculty"} className="border rounded w-full px-3 py-2 bg-white">
          ...
          <Field label="Address"><textarea name="address" rows={2} defaultValue={v.address ?? ""} className="border rounded w-full px-3 py-2 resize-y" /></Field>

          <div className="border-t pt-4 grid sm:grid-cols-2 gap-4">
            <Field label="Login Username"><input name="username" defaultValue={v.username ?? ""} required className="border rounded w-full px-3 py-2" /></Field>
            {!isEdit && <Field label="Password (min 8)"><input name="password" type="password" required minLength={8} className="border rounded w-full px-3 py-2" /></Field>}
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button disabled={busy} className="bg-[#7b1f4c] text-white px-5 py-2 rounded font-semibold disabled:opacity-50">{busy ? "Saving…" : isEdit ? "Save Changes" : "Create Faculty"}</button>
          </div>
        </form>
      </div>
    </div>
