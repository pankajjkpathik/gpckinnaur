...
import { z } from "zod";
import { BarChart3, Users, ClipboardCheck, FileSpreadsheet, BookMarked, Calendar, ArrowLeft, Check, RotateCcw, ChevronDown, Home, LayoutDashboard, Shield, Mail, Trash2, Plus, GraduationCap, Building2, RefreshCw, PanelLeftClose, PanelLeftOpen, Download, } from "lucide-react";
...
import { hodDashboard, hodMarksGroups, hodMarksDetail, hodReviewMarks, hodExportApprovedMarks, deptClassAttendance, hodListAssignments, hodUpsertAssignment, hodDeleteAssignment, } from "@/lib/hod.functions";
import { hodDepartmentOverview } from "@/lib/tpo.functions";
import { listSubjects, listStaffByRole } from "@/lib/academic.functions";
...
function BackBtn({ onClick }) {
    return (<button onClick={onClick} className="inline-flex items-center gap-1.5 text-sm text-gray-600 border rounded px-3 py-1.5 mb-5 hover:bg-gray-50 bg-white">
...
function SidebarGroup({ title, icon: Icon, gradient, items, active, onNav, defaultOpen, canWrite, }) {
    ...
    if (visible.length === 0)
        return null;
    return (<div>
      <button onClick={() => setOpen((o) => !o)} className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg ${gradient} text-white text-sm font-semibold shadow-sm`}>
        <span className="flex items-center gap-2">
          <Icon className="w-4 h-4"/> {title}
        </span>
        <ChevronDown className={`w-4 h-4 transition-transform ${open ? "" : "-rotate-90"}`}/>
      </button>
      {open && (<ul className="mt-1.5 space-y-0.5">
          {visible.map((item) => {
                const isActive = active === item.view;
                return (<li key={item.view}>
                <button onClick={() => onNav(item.view)} className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${isActive
...
function HodSidebar({ active, onNav, mobileOpen, onCloseMobile, collapsed, onToggleCollapsed, canWrite, }) {
    ...
    const collapsedRail = (<nav className="bg-white border border-gray-200 rounded-xl shadow-sm p-2 flex flex-col items-center gap-1" aria-label="HOD navigation (collapsed)">
      <button onClick={onToggleCollapsed} className="w-10 h-10 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-100" title="Expand sidebar" aria-label="Expand sidebar">
        <PanelLeftOpen className="w-4 h-4"/>
      </button>
      <div className="w-8 border-t border-gray-100 my-1"/>
      {[...hodNav, ...monitoringNav].map((item) => {
            const isActive = active === item.view;
            return (<button key={item.view} onClick={() => onNav(item.view)} title={item.label} aria-label={item.label} className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${isActive
      ...
      <div className="w-8 border-t border-gray-100 my-1"/>
      <Link to="/faculty" title="Faculty Portal" aria-label="Faculty Portal" className="w-10 h-10 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-50 hover:text-gray-900">
        <GraduationCap className="w-4 h-4"/>
      </Link>
      <a href="/profile-settings" title="My Profile" aria-label="My Profile" className="w-10 h-10 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-50 hover:text-gray-900">
        <LayoutDashboard className="w-4 h-4"/>
      </a>
      <a href="/messages" title="Internal Messages" aria-label="Internal Messages" className="w-10 h-10 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-50 hover:text-gray-900">
        <Mail className="w-4 h-4"/>
      </a>
    </nav>);
    const inner = (<nav className="bg-white border border-gray-200 rounded-xl shadow-sm p-3 space-y-2">
      <div className="hidden lg:flex justify-end -mt-1 -mr-1 mb-1">
        <button onClick={onToggleCollapsed} className="p-1.5 rounded-md text-gray-400 hover:bg-gray-100 hover:text-gray-700" title="Collapse sidebar" aria-label="Collapse sidebar">
          <PanelLeftClose className="w-4 h-4"/>
        </button>
      </div>

      <SidebarGroup title="HOD Portal" icon={LayoutDashboard} gradient="bg-gradient-to-r from-[#7b1f4c] to-[#a63a6b]" items={HOD_NAV} active={active} onNav={onNav} defaultOpen canWrite={canWrite}/>

      <SidebarGroup title="Academic Monitoring" icon={BarChart3} gradient="bg-gradient-to-r from-[#5b1138] to-[#7b1f4c]" items={MONITORING_NAV} active={active} onNav={onNav} canWrite={canWrite}/>

      <div className="pt-1 border-t border-gray-100">
        <p className="px-3 pt-2 pb-1 text-[10px] uppercase tracking-[0.18em] text-gray-400 font-semibold">General</p>
        <ul className="space-y-0.5">
          <li>
            <Link to="/faculty" className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900">
              <GraduationCap className="w-4 h-4 text-gray-400"/> Faculty Portal
            </Link>
          </li>
          <li>
            <a href="https://www.gpkinnaur.edu.in" target="_blank" rel="noopener noreferrer" className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900">
              <Home className="w-4 h-4 text-gray-400"/> Public Website
            </a>
          </li>
          <li>
            <a href="/profile-settings" className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900">
              <LayoutDashboard className="w-4 h-4 text-gray-400"/> My Profile
            </a>
          </li>
          <li>
            <a href="/staff-change-password" className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900">
              <Shield className="w-4 h-4 text-gray-400"/> Change Password
            </a>
          </li>
          <li>
            <a href="/messages" className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900">
    ...
    return (<>
      <aside className={`hidden lg:block shrink-0 sticky top-[76px] self-start transition-[width] duration-200 ${collapsed ? "w-14" : "w-64"}`}>
        {collapsed ? collapsedRail : inner}
      </aside>
      {mobileOpen && (<div className="lg:hidden fixed inset-0 z-40 bg-black/40" onClick={onCloseMobile}>
          <div className="absolute left-0 top-0 h-full w-72 bg-[#f7f7fb] p-4 overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
...
function HodPortal() {
    ...
    useEffect(() => {
        ...
        else if (!hasRole(me, hodRoles))
            nav({ to: "/staff-dashboard", replace: true });
    }, [me, isLoading, nav]);
    const VALID_VIEWS = ["home", "overview", "faculty", "attendance", "sessional", "syllabus", "timetable", "lessons"];
    const initialView = (() => {
    ...
    if (isLoading || !me || !hasRole(me, hodRoles))
    ...
    const WRITE_ONLY_VIEWS = ["faculty", "lessons"];
    ...
    return (<div className="min-h-screen bg-[#f7f7fb]">
      <header className="bg-white border-b sticky top-0 z-30">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => setMobileNavOpen((o) => !o)} className="lg:hidden -ml-1 p-2 rounded hover:bg-gray-100" aria-label="Toggle navigation">
      ...
      <div className="container mx-auto px-4 py-6 flex gap-6">
        <HodSidebar active={view} onNav={navTo} mobileOpen={mobileNavOpen} onCloseMobile={() => setMobileNavOpen(false)} collapsed={sidebarCollapsed} onToggleCollapsed={() => setSidebarCollapsed((c) => !c)} canWrite={!isViewer}/>
        <main className="flex-1 min-w-0">
          {isViewer && view !== "home" && (<div className="bg-amber-50 border border-amber-200 text-amber-900 text-xs rounded px-3 py-2 mb-4">
              You are viewing the HOD portal as <strong className="capitalize">{me.role}</strong>. Approvals and edits are disabled.
            </div>)}
          <fieldset disabled={isViewer && (view === "sessional" || view === "faculty" || view === "timetable" || view === "lessons")} className={isViewer && (view === "sessional" || view === "faculty" || view === "timetable" || view === "lessons")
            ? "pointer-events-none opacity-90"
            : ""}>
            {view === "home" && <HomeView me={me} deptLabel={deptLabel} ay={ay}/>}
            {view === "overview" && (<OverviewView branch={branch} ay={ay} deptLabel={deptLabel} onBack={() => setView("home")}/>)}
            {view === "faculty" && (<FacultyAllotmentView branch={branch} deptLabel={deptLabel} onBack={() => setView("home")}/>)}
            {view === "attendance" && (<AttendanceReportsView defaultBranch={branch} onBack={() => setView("home")}/>)}
            {view === "sessional" && <SessionalReportsView ay={ay} branch={branch} deptLabel={deptLabel} onBack={() => setView("home")}/>}
            {view === "syllabus" && (<SyllabusProgressView branch={branch} deptLabel={deptLabel} ay={ay} onBack={() => setView("home")}/>)}
            {view === "timetable" && (<TimetableView branch={branch} ay={ay} editable={!isViewer} onBack={() => setView("home")}/>)}
...
/* ─── HOME ───────────────────────────────────────────────────────────────── */
function HomeView({ me, deptLabel, ay }) {
    ...
    return (<div className="space-y-6">
      <HeroBanner name={me?.name || me?.username || "HOD"} role={`HOD · ${deptLabel}`} palette="hod" avatarSrc={avatarUrl(me)} fallbackAvatarSrc={placeholderAvatar} subtitle={<>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {snapshots.map((s) => (<div key={s.label} className={`rounded-xl border p-4 ${s.tint}`}>
...
/* ─── DEPARTMENT OVERVIEW ────────────────────────────────────────────────── */
function OverviewView({ branch, ay, deptLabel, onBack, }) {
    const q = useQuery({
    ...
    return (<div className="space-y-5">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#065f46] via-[#064e3b] to-[#052e2b] text-white shadow-lg">
        ...
        <div className="relative p-5 sm:p-6 flex flex-wrap items-start gap-4">
          ...
          <button type="button" onClick={() => q.refetch()} disabled={isBusy} className="shrink-0 inline-flex items-center gap-1.5 rounded-lg bg-white text-emerald-900 px-3 py-2 text-xs font-semibold shadow-sm hover:bg-emerald-50 disabled:opacity-60 disabled:cursor-not-allowed">
      ...
      {q.isLoading || !q.data ? (<OverviewSkeleton />) : (<DepartmentOverviewPanel d={q.data}/>)}
    </div>);
}
function OverviewSkeleton() {
    return (<div className="space-y-5">
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[0, 1, 2, 3].map((i) => (<div key={i} className="h-24 rounded-xl border bg-gradient-to-br from-gray-100 to-gray-50 animate-pulse"/>))}
      </div>
      <div className="h-56 rounded-xl border bg-white p-5">
        <div className="h-4 w-40 bg-gray-100 rounded animate-pulse"/>
        <div className="mt-4 grid grid-cols-4 gap-4">
          {[0, 1, 2, 3].map((i) => (<div key={i} className="h-32 bg-gradient-to-t from-gray-100 to-gray-50 rounded animate-pulse"/>))}
        </div>
      </div>
...
function FacultyAllotmentView({ branch, deptLabel, onBack, }) {
    ...
    const staffQ = useQuery({
        ...
        queryFn: () => listStaffByRole({ data: {} }),
    });
    const subjQ = useQuery({
        queryKey: ["subjects-of", branch, form.semester],
        queryFn: () => listSubjects({ data: { branch, semester: form.semester } }),
        enabled: !!branch && !!form.semester,
    });
    const save = useMutation({
        mutationFn: (d) => hodUpsertAssignment({ data: d }),
        onSuccess: () => {
    ...
    const del = useMutation({
        mutationFn: (id) => hodDeleteAssignment({ data: { id } }),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ["hod-assignments"] });
            toast.success("Allotment removed");
        },
        onError: (e) => toast.error(e?.message || "Failed to remove allotment"),
    });
    ...
    const facultyPool = (staffQ.data ?? []).filter((s) => ["faculty", "hod"].includes(s.role) || (s.extra_roles ?? []).includes("faculty"));
    const own = facultyPool.filter((s) => deptToBranch(s.department) === branch);
    const guests = facultyPool.filter((s) => deptToBranch(s.department) !== branch);
    ...
    return (<div className="space-y-4">
      ...
      <Card>
        ...
        ].map(([v, l]) => (<button key={v} type="button" onClick={() => {
                setFormError(null);
                setForm((f) => ({ ...f, mode: v, staff_id: 0, guest_faculty: "", guest_institute: "" }));
            }} className={`px-3 py-1.5 rounded border font-semibold ${form.mode === v
        ...
        <form onSubmit={(e) => {
            ...
            const dupe = (assignQ.data ?? []).some((a) => a.subject_id === form.subject_id && a.semester === form.semester
                ? isExternal
          ...
          <label className="text-xs">
            1. Branch
            <input value={deptLabel} disabled className="w-full border rounded px-2 py-1.5 text-sm bg-gray-100"/>
          </label>
          <label className="text-xs">
            2. Semester
            <select value={form.semester} onChange={(e) => {
            setFormError(null);
            setForm({ ...form, semester: Number(e.target.value), subject_id: 0 });
        }} required className="w-full border rounded px-2 py-1.5 text-sm bg-white">
              <option value={0}>— select —</option>
              {[1, 2, 3, 4, 5, 6].map((s) => (<option key={s} value={s}>
                  Sem {s}
                </option>))}
            </select>
          </label>
        ...
        }} required disabled={!form.semester} className="w-full border rounded px-2 py-1.5 text-sm bg-white disabled:bg-gray-100">
              <option value={0}>
                {!form.semester
            ? "— pick semester first —"
            : (subjQ.data ?? []).length === 0
              ...
              {(subjQ.data ?? []).map((s) => (<option key={s.id} value={s.id}>
                  {s.code} · {s.name}
                </option>))}
            </select>
          </label>
          {form.mode === "internal" ? (<label className="text-xs">
              4. Faculty
              <select value={form.staff_id} onChange={(e) => {
                setFormError(null);
                setForm({ ...form, staff_id: Number(e.target.value) });
            }} required disabled={!form.subject_id} className="w-full border rounded px-2 py-1.5 text-sm bg-white disabled:bg-gray-100">
                <option value={0}>— select —</option>
                {own.length > 0 && (<optgroup label={`Department — ${deptLabel}`}>
                    {own.map((s) => (<option key={s.id} value={s.id}>
                        {s.name || s.username} ({s.role})
                      </option>))}
                  </optgroup>)}
                {guests.length > 0 && (<optgroup label="Guest Faculty (other departments)">
                    {guests.map((s) => (<option key={s.id} value={s.id}>
              ...
              <label className="text-xs">
                ...
                <input value={form.guest_faculty} onChange={(e) => {
                setFormError(null);
                setForm({ ...form, guest_faculty: e.target.value });
            }} placeholder="e.g. Er. R. K. Sharma" disabled={!form.subject_id} className="w-full border rounded px-2 py-1.5 text-sm bg-white disabled:bg-gray-100"/>
              </label>
              <label className="text-xs">
                5. Institute / Polytechnic
                <input value={form.guest_institute} onChange={(e) => setForm({ ...form, guest_institute: e.target.value })} placeholder="e.g. GP Rampur" disabled={!form.subject_id} className="w-full border rounded px-2 py-1.5 text-sm bg-white disabled:bg-gray-100"/>
              </label>
            </>)}
          <button disabled={save.isPending ||
            (form.mode === "internal" ? !form.staff_id : !form.guest_faculty.trim())} className="bg-[#7b1f4c] text-white rounded px-3 py-2 text-sm font-semibold inline-flex items-center gap-1 justify-center disabled:opacity-50">
            <Plus className="w-4 h-4"/> {save.isPending ? "Allotting…" : "Allot"}
          </button>

          {formError && (<p className="col-span-full text-xs text-rose-700" role="alert">
              {formError}
            </p>)}
        </form>
      </Card>
            ...
            return (<tr key={a.id} className="border-t">
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        {isExternal && (<span className="text-[10px] font-semibold bg-indigo-100 text-indigo-700 rounded px-1.5 py-0.5">
                            EXTERNAL GUEST
                          </span>)}
                        {isGuest && (<span className="text-[10px] font-semibold bg-amber-100 text-amber-700 rounded px-1.5 py-0.5">
                            GUEST
                          </span>)}
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2 text-right">
                      <button onClick={() => confirm("Remove this allotment?") && del.mutate(a.id)} className="p-1.5 text-rose-600 hover:bg-rose-50 rounded" title="Remove">
                        <Trash2 className="w-4 h-4"/>
                      </button>
                    </td>
                  </tr>);
        })}
              {(assignQ.data ?? []).length === 0 && (<tr>
...
/* ─── ATTENDANCE REPORTS ─────────────────────────────────────────────────── */
function AttendanceReportsView({ defaultBranch = "", onBack }) {
    ...
    const rows = (q.data ?? []).map((s) => [s.enrollment_no, s.name, s.present, s.total, `${s.pct}%`]);
    ...
    return (<div className="space-y-4">
      ...
      <Card>
        ...
        <div className="grid sm:grid-cols-4 gap-2 text-sm mb-2">
          ...
          <select value={sem} onChange={(e) => setSem(e.target.value ? Number(e.target.value) : "")} className="border rounded px-3 py-2 bg-white">
        ...
        {rows.length > 0 && (<div className="flex gap-2 mb-3">
            <button onClick={() => exportPDF(fileBase, title, `${from} to ${to}`, header, rows)} className="text-xs bg-rose-600 text-white px-3 py-1.5 rounded">
              PDF
            </button>
            <button onClick={() => exportExcel(fileBase, "Attendance", header, rows)} className="text-xs bg-green-700 text-white px-3 py-1.5 rounded">
              Excel
            </button>
            <button onClick={() => exportCSV(fileBase, header, rows)} className="text-xs bg-gray-100 px-3 py-1.5 rounded">
              CSV
            </button>
          </div>)}
        {q.data && (<div className="border rounded overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  {header.map((h) => (<th key={h} className="text-left px-4 py-3 text-gray-400 font-medium">
                      {h}
                    </th>))}
                </tr>
              </thead>
              <tbody>
                ...
                return (<tr key={i} className={`border-t ${pct < 75 ? "bg-rose-50" : ""}`}>
                      {r.map((c, j) => (<td key={j} className="px-4 py-3">
                          {c}
                        </td>))}
                    </tr>);
            })}
...
function SessionalReportsView({ ay, branch, deptLabel, onBack, }) {
    ...
    const runExport = async (format) => {
        ...
        try {
            ...
            if (format === "csv") {
                ...
                exportPDF(filename, `${tabLabel} Sessional Marks — ${deptLabel}`, `${filterSummary} · ${rows.length} record${rows.length === 1 ? "" : "s"}`, header, body);
            }
        }
        catch (e) {
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <label className="text-[11px] text-gray-500 inline-flex items-center gap-1">
              Exam
              <select value={examType} onChange={(e) => setExamType(e.target.value)} className="border rounded px-2 py-1 text-xs bg-white">
            ...
            <div className="inline-flex items-center gap-1.5">
              <button onClick={() => runExport("csv")} disabled={exporting !== null} title={`Download ${tab} sessional marks · ${filterSummary} as CSV`} className="text-xs px-3 py-1.5 rounded border border-gray-300 bg-white hover:bg-gray-50 disabled:opacity-50">
                {exporting === "csv" ? "Exporting…" : "Export CSV"}
              </button>
              <button onClick={() => runExport("pdf")} disabled={exporting !== null} title={`Download ${tab} sessional marks · ${filterSummary} as PDF`} className="text-xs px-3 py-1.5 rounded bg-[#7b1f4c] text-white hover:bg-[#651841] disabled:opacity-50">
                {exporting === "pdf" ? "Exporting…" : "Export PDF"}
              </button>
            </div>
            <div className="inline-flex rounded-lg border overflow-hidden text-xs">
              {["pending", "approved", "returned"].map((t) => (<button key={t} onClick={() => setTab(t)} className={`px-3 py-1.5 capitalize ${tab === t ? "bg-[#7b1f4c] text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}>
                  {t}
                </button>))}
            </div>
          </div>
...
function ProvenanceBadge({ row, status }) {
    ...
    return (<span title={tooltip} className={`inline-flex flex-col items-start gap-0.5 px-2 py-1 rounded border text-[10px] leading-tight ${cls}`}>
      <span className="font-semibold uppercase tracking-wide">{label}</span>
      {status !== "pending" && (who || when) && (<span className="text-[9px] opacity-80">
          {who ? `by ${who}` : ""}
          {who && when ? " · " : ""}
          {when ?? ""}
        </span>)}
    ...
    return (<div>
      <div className="border rounded overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {(q.data ?? []).map((b, i) => (<tr key={i} className="border-t">
      ...
      {open && detail.data && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setOpen(null)}>
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {detail.data.map((r) => (<tr key={r.id} className="border-t">
            ...
            <div className="p-3 border-t flex flex-wrap gap-2 justify-between items-center">
              ...
              {status === "pending" && (<>
                  <button onClick={() => {
                    ...
                    if (r != null)
                        review.mutate({ decision: "returned", remarks: r });
                }} className="text-sm bg-amber-600 text-white px-3 py-1.5 rounded">
                    Return for Revision
                  </button>
                  <button onClick={() => review.mutate({ decision: "approved" })} className="text-sm bg-green-600 text-white px-3 py-1.5 rounded">
                    Approve &amp; Lock
                  </button>
                </>)}
...
/* ─── SYLLABUS COVERAGE ──────────────────────────────────────────────────── */
function SyllabusProgressView({(branch,
            deptLabel,
            ay,
            onBack,
        )}: {branch}: string;
...
function TimetableView({(branch,
  ...
  const ttQ = useQuery({queryKey}: ["hod-tt", branch, sem, ay],
    queryFn: () => listTimetable({data}: {(branch, semester)}: sem, academic_year: ay } }),
    enabled: !!branch,
  });
  const subjQ = useQuery({queryKey}: ["subjects-of", branch, sem],
    queryFn: () => listSubjects({data}: {(branch, semester)}: sem } as any }),
    enabled: !!branch,
  });
  const staffQ = useQuery({queryKey}: ["staff-all"],
    queryFn: () => listStaffByRole({data}:  as any }),
  });
  const save = useMutation({mutationFn}: (d: any) =>
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        ...
        {save.error && (<p className="text-xs text-rose-700 bg-rose-50 border border-rose-200 rounded p-2 mb-2">
            {save.error.message}
          </p>)}
        {(periodsQ.data ?? []).length === 0 ? (<p className="text-center py-8 text-gray-400 text-sm">
            Periods Master not configured yet. Ask Admin to set it up.
          </p>) : (<TimetableGrid periods={periodsQ.data} slots={(ttQ.data ?? [])} subjects={subjQ.data} staff={staffQ.data} editable={editable} onSaveSlot={(p) => save.mutate(p)} institutionLine="Govt. Polytechnic Kinnaur, Camp at GP Rohru Distt. Shimla (H.P.)" classLine={`${branchToDept(branch)} - ${ORD[sem]} Semester`}/>)}
      </Card>
    </div>
  );
}
...
function LessonsReviewView({(ay, onBack)}: {ay}: string; onBack: () => void }) {}
  const qc = useQueryClient();
  const q = useQuery({queryKey}: ["hod-lessons", ay],
    queryFn: () => hodPendingLessonPlans({data}: {academic_year}: ay } }),
  });
  const review = useMutation({mutationFn}: (v: {id}: number; decision: "approved" | "returned"; hod_remarks?: string }) =>
      ...
      <Card>
        ...
        <div className="border rounded overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(q.data ?? []).map((p) => (<tr key={p.id} className="border-t">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <button onClick={() => review.mutate({ id: p.id, decision: "approved" })} className="text-xs bg-green-600 text-white px-2 py-1 rounded inline-flex items-center gap-1">
                ...
                if (r != null)
                    review.mutate({ id: p.id, decision: "returned", hod_remarks: r });
