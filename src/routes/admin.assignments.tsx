...
function AssignmentsPage() {
  ...
  const save = useMutation({ mutationFn: (d: any) => upsertAssignment({ data: d }), onSuccess: () => qc.invalidateQueries({ queryKey: ["assignments"] }) });
    ...
    <PortalShell title="Faculty Assignments" subtitle="Admin · Teaching Allocation" me={me as any} accent="rose">
      <div className="container mx-auto px-4 py-6 space-y-4">
        ...
        <form onSubmit={(e) => {
          ...
          <label className="text-xs">Faculty
            <select value={form.staff_id} onChange={(e) => setForm({ ...form, staff_id: Number(e.target.value) })} required className="w-full border rounded px-2 py-1.5 text-sm bg-white">
              <option value={0}>—</option>
              {(staffQ.data ?? []).filter((s: any) => ["faculty", "hod"].includes(s.role)).map((s: any) => <option key={s.id} value={s.id}>{s.username} ({s.role})</option>)}
            </select>
          </label>
          <label className="text-xs">Subject
            <select value={form.subject_id} onChange={(e) => {
              ...
              setForm({ ...form, subject_id: id, branch: subj?.branch ?? form.branch, semester: subj?.semester ?? form.semester });
            }} required className="w-full border rounded px-2 py-1.5 text-sm bg-white">
              <option value={0}>—</option>
              {(subjQ.data ?? []).map((s: any) => <option key={s.id} value={s.id}>{s.code} · {s.name}</option>)}
            </select>
          </label>
          <label className="text-xs">Branch
            <input value={form.branch} onChange={(e) => setForm({ ...form, branch: e.target.value })} required className="w-full border rounded px-2 py-1.5 text-sm" />
          </label>
          <label className="text-xs">Sem
            <input type="number" min={1} max={8} value={form.semester} onChange={(e) => setForm({ ...form, semester: Number(e.target.value) })} required className="w-full border rounded px-2 py-1.5 text-sm" />
          </label>
          <button disabled={save.isPending} className="bg-rose-700 text-white rounded px-3 py-2 text-sm font-semibold inline-flex items-center gap-1 justify-center disabled:opacity-50">
              ...
              {(assignQ.data ?? []).map((a: any) => (
                <tr key={a.id} className="border-t">
                  ...
                  <td className="px-3 py-2 text-right">
                    <button onClick={() => confirm("Remove?") && del.mutate(a.id)} className="p-1.5 text-destructive hover:bg-destructive/10 rounded"><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))}
