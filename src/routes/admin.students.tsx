...
function StudentManagement() {
  ...
  const rows = (listQ.data ?? []).filter((s: any) => {
    ...
    return (s.name || "").toLowerCase().includes(q) || (s.enrollment_no || "").toLowerCase().includes(q) ||
      (s.father_name || "").toLowerCase().includes(q) || (s.aadhaar_number || "").includes(search);
  });
    ...
    <PortalShell title="Student Management" subtitle="Student records & details" me={me as any} accent="rose">
        ...
        <div className="bg-white border rounded-lg p-5">
          <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
            ...
            <div className="flex gap-2">
              <select value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)} className="border rounded px-3 py-2 text-sm bg-white">
                <option value="">All branches</option>
                {BRANCHES.map((b) => <option key={b} value={b}>{b}</option>)}
              </select>
              <div className="relative">
                <Search className="w-4 h-4 text-gray-400 absolute left-2.5 top-2.5" />
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, enrollment…" className="border rounded pl-8 pr-3 py-2 text-sm w-56" />
              </div>
            </div>
          </div>
                ...
                {rows.map((s: any) => (
                  <tr key={s.id} className="border-t">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-gray-400 font-mono">{s.enrollment_no}{s.is_active ? "" : " · inactive"}</p>
                    </td>
                    <td className="px-4 py-3">{s.father_name || s.guardian_name || "—"}</td>
                    <td className="px-4 py-3 capitalize text-xs">{s.branch}-Sem{s.semester}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2 items-center">
                        <button onClick={() => setEditing(s)} className="text-gray-500 hover:text-gray-700" title="Edit"><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => { const p = prompt(`New password for ${s.enrollment_no} (min 6 chars):`); if (p) resetPw.mutate({ id: s.id, newPassword: p }); }} className="text-gray-500 hover:text-gray-700" title="Reset password"><KeyRound className="w-4 h-4" /></button>
                        <button onClick={() => { if (confirm(`Delete ${s.name}? This removes their login too.`)) del.mutate(s.id); }} className="text-rose-500 hover:text-rose-700" title="Delete"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {rows.length === 0 && (
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">{listQ.isLoading ? "Loading…" : "No students found."}</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {(creating || editing) && (
        <StudentForm
          ...
          onSaved={() => { qc.invalidateQueries({ queryKey: ["student-list"] }); setCreating(false); setEditing(null); }}
        />
      )}
    </PortalShell>
  );
...
function StudentForm({ initial, onClose, onSaved }: { initial: Student | null; onClose: () => void; onSaved: () => void }) {
      ...
      <div className="bg-white rounded-lg w-full max-w-3xl my-8" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} className="p-5 space-y-4 text-sm">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <Field label="Name"><input name="name" defaultValue={v.name ?? ""} required className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Father's Name"><input name="father_name" defaultValue={v.father_name ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            ...
            <Field label="Semester">
              <select name="semester" defaultValue={v.semester ?? 1} required className="border rounded w-full px-3 py-2 bg-white">
                {[1,2,3,4,5,6,7,8].map((n) => <option key={n} value={n}>{n}</option>)}
              </select>
            </Field>
            <Field label="Batch Year"><input name="batch_year" type="number" defaultValue={v.batch_year ?? thisYear} required min={2000} max={2100} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Aadhaar Number"><input name="aadhaar_number" defaultValue={v.aadhaar_number ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Bank Account Number"><input name="bank_account_number" defaultValue={v.bank_account_number ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Student Phone"><input name="phone" defaultValue={v.phone ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Parent Mobile Number"><input name="parent_phone" defaultValue={v.parent_phone ?? ""} className="border rounded w-full px-3 py-2" /></Field>
            <Field label="Email ID"><input name="email" type="email" defaultValue={v.email ?? ""} className="border rounded w-full px-3 py-2" /></Field>
          </div>
          ...
          <div className="border-t pt-4 grid sm:grid-cols-2 gap-4">
            <Field label="Enrollment No."><input name="enrollment_no" defaultValue={v.enrollment_no ?? ""} required className="border rounded w-full px-3 py-2" /></Field>
            {!isEdit && <Field label="Password (min 6)"><input name="password" type="password" required minLength={6} className="border rounded w-full px-3 py-2" /></Field>}
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button disabled={busy} className="bg-[#7b1f4c] text-white px-5 py-2 rounded font-semibold disabled:opacity-50">{busy ? "Saving…" : isEdit ? "Save Changes" : "Create Student"}</button>
          </div>
        </form>
      </div>
    </div>
