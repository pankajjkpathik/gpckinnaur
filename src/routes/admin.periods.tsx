...
function PeriodsPage() {
  ...
  const save = useMutation({ mutationFn: (d: any) => upsertPeriod({ data: d }), onSuccess: () => qc.invalidateQueries({ queryKey: ["periods"] }) });
  ...
  return (
    <PortalShell title="Periods Master" subtitle="Admin · Bell Schedule" me={me as any} accent="rose">
      <div className="container mx-auto px-4 py-6 space-y-4">
        ...
        >
          <label className="text-xs">Period # <input type="number" min={1} max={12} value={next.period_no} onChange={(e) => setNext({ ...next, period_no: Number(e.target.value) })} className="w-full border rounded px-2 py-1.5" /></label>
          <label className="text-xs">Start <input type="time" value={next.start_time} onChange={(e) => setNext({ ...next, start_time: e.target.value })} className="w-full border rounded px-2 py-1.5" /></label>
          <label className="text-xs">End <input type="time" value={next.end_time} onChange={(e) => setNext({ ...next, end_time: e.target.value })} className="w-full border rounded px-2 py-1.5" /></label>
          <label className="text-xs">Label <input value={next.label} onChange={(e) => setNext({ ...next, label: e.target.value })} placeholder="e.g. Lunch" className="w-full border rounded px-2 py-1.5" /></label>
          <label className="text-xs flex items-center gap-1"><input type="checkbox" checked={next.is_break} onChange={(e) => setNext({ ...next, is_break: e.target.checked })} /> Break</label>
              ...
              {(periodsQ.data ?? []).map((p: any) => (
                <tr key={p.id} className="border-t">
                  ...
                  <td className="px-3 py-2 text-right">
                    <button onClick={() => confirm("Delete period?") && del.mutate(p.id)} className="p-1.5 text-destructive hover:bg-destructive/10 rounded"><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))}
