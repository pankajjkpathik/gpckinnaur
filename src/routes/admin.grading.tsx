      ...
      <div className="container mx-auto px-4 py-6 space-y-4 max-w-3xl">
        <div className="flex items-start justify-between gap-2 flex-wrap">
          <p className="text-sm text-muted-foreground">
            Define grade boundaries from highest to lowest. Used when results are computed.
          </p>
          <BulkOpsBar
            sample={DEFAULT_ROWS}
          ...
          <table className="w-full text-sm">
            <thead className="bg-secondary">
              <tr>
                ...
                <th className="px-2 py-2 text-left">Grade Pt</th>
                ...
                <tr key={i} className="border-t">
                  <td className="px-2 py-1">
                    <input
                      ...
                      step="0.01"
                      value={r.min_pct}
                      onChange={(e) => upd(i, "min_pct", Number(e.target.value))}
                  ...
                  <td className="px-2 py-1">
                    <input
                      ...
                      step="0.01"
                      value={r.max_pct}
                      onChange={(e) => upd(i, "max_pct", Number(e.target.value))}
                      className="w-20 border rounded px-2 py-1"
                    />
                  </td>
                  <td className="px-2 py-1">
                  ...
                  <td className="px-2 py-1">
                    <input
                      ...
                      step="0.01"
                      value={r.grade_point}
                      onChange={(e) => upd(i, "grade_point", Number(e.target.value))}
                      className="w-20 border rounded px-2 py-1"
                    />
                  </td>
                  <td className="px-2 py-1">
                    <input type="checkbox" checked={r.is_pass} onChange={(e) => upd(i, "is_pass", e.target.checked)} />
                  </td>
        ...
        <div className="flex justify-between">
          <button
            onClick={() => setRows([...rows, { min_pct: 0, max_pct: 100, grade: "?", grade_point: 0, is_pass: true }])}
          ...
          <button
            ...
            className="bg-rose-700 text-white px-4 py-2 rounded text-sm font-semibold inline-flex items-center gap-1 disabled:opacity-50"
          >
            <Save className="w-4 h-4" /> {save.isPending ? "Saving…" : "Save Scheme"}
          </button>
        </div>
