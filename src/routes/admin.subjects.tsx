    ...
    <PortalShell title="Subjects" subtitle="Admin · Master Data" me={me as any} accent="rose">
      <div className="container mx-auto px-4 py-6 space-y-4">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <button
            onClick={() => setEditing({ kind: "theory", credits: 4, semester: 1, branch: "civil", category: "PCC" })}
            ...
            <thead className="bg-secondary">
              <tr>
                <th className="px-2 py-2">
                  <input
                    type="checkbox"
                    checked={selected.size > 0 && selected.size === (subjectsQ.data ?? []).length}
                    onChange={toggleAll}
                  />
                </th>
              ...
              {(subjectsQ.data ?? []).map((s: any) => (
                <tr key={s.id} className="border-t">
                  ...
                  <td className="px-2 py-2 flex gap-1 justify-end">
                    <button onClick={() => setEditing(s)} className="p-1.5 hover:bg-secondary rounded">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => confirm("Delete subject?") && del.mutate(s.id)}
        ...
        >
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              required
              placeholder="Subject Code (e.g. CE301)"
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            <select
              name="semester"
              defaultValue={String(initial.semester ?? 1)}
              required
              className="border rounded px-3 py-2 text-sm bg-white"
            >
              {[1, 2, 3, 4, 5, 6].map((s) => (
            ...
            <select
              name="category"
              defaultValue={initial.category ?? "PCC"}
              className="border rounded px-3 py-2 text-sm bg-white"
            >
            ...
            <select
              name="kind"
              defaultValue={initial.kind ?? "theory"}
              className="border rounded px-3 py-2 text-sm bg-white"
            >
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              disabled={pending}
              className="px-4 py-1.5 bg-rose-700 text-white rounded text-sm disabled:opacity-50"
            >
              {pending ? "Saving…" : "Save"}
            </button>
          </div>
